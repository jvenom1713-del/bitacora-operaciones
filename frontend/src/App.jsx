...
export default function App() {
  ...
  // ── MODO DEMO: BARRA DE NAVEGACIÓN RÁPIDA ──────────────────────────────────
  const [minimizadoDemoBar, setMinimizadoDemoBar] = useState(false);

  const demoBarra = (
    <div className="fixed bottom-3 left-1/2 -translate-x-1/2 z-[9999] print:hidden max-w-[98vw]">
      {minimizadoDemoBar ? (
        <button
          type="button"
          onClick={() => setMinimizadoDemoBar(false)}
          className="flex items-center gap-2 bg-slate-900/95 hover:bg-slate-800 text-amber-400 font-extrabold text-xs px-3.5 py-2 rounded-full border-2 border-amber-500/60 shadow-2xl backdrop-blur-md cursor-pointer transition-all hover:scale-105"
          title="Abrir Barra Demo de Navegación Rápida"
        >
          <Compass className="w-4 h-4 text-amber-400 animate-spin-slow" />
          <span>Navegador Demo</span>
          <ChevronUp className="w-3.5 h-3.5 text-slate-400 ml-0.5" />
        </button>
      ) : (
        <div className="bg-slate-900/95 border-2 border-slate-700 text-white rounded-2xl p-2 shadow-2xl backdrop-blur-lg flex items-center gap-1.5 overflow-x-auto max-w-full ring-1 ring-white/10">
          <div className="flex items-center gap-1 px-2 border-r border-slate-700/80 mr-0.5 shrink-0">
            <Compass className="w-4 h-4 text-amber-400" />
            <span className="text-[11px] font-black tracking-wider uppercase text-amber-400 hidden sm:inline">DEMO</span>
          </div>

          <button
            type="button"
            onClick={() => volverMenuGenerico()}
            className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 ${
              vistaActual === 'MENU_OPERADOR' || vistaActual === 'MENU_JEFE'
          ...
          <button
            type="button"
            onClick={() => setVistaActual('ABRIR_TURNO_MENU')}
            className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 ${
              vistaActual === 'ABRIR_TURNO_MENU'
                ? 'bg-amber-500 text-slate-950 shadow-md font-black'
                : 'bg-slate-800/80 text-slate-300 hover:bg-slate-700 hover:text-white'
            }`}
            title="Ir a Iniciar Turno"
          >
          ...
          <button
            type="button"
            onClick={() => setVistaActual('BITACORA_DASHBOARD')}
            className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 ${
              vistaActual === 'BITACORA_DASHBOARD'
                ? 'bg-amber-500 text-slate-950 shadow-md font-black'
                : 'bg-slate-800/80 text-slate-300 hover:bg-slate-700 hover:text-white'
            }`}
            title="Ir a Bitácora Operacional"
          >
          ...
          <button
            type="button"
            onClick={() => {
              setVistaAnteriorCambioPersonal(vistaActual);
              setVistaActual('CAMBIO_PERSONAL_MENU');
            }}
            className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 ${
              vistaActual === 'CAMBIO_PERSONAL_MENU'
                ? 'bg-amber-500 text-slate-950 shadow-md font-black'
                : 'bg-slate-800/80 text-slate-300 hover:bg-slate-700 hover:text-white'
            }`}
            title="Ir a Cambio de Personal de Turno"
          >
            <Users className="w-3.5 h-3.5" />
            <span>Cambio Personal</span>
          </button>

          <button
            type="button"
            onClick={() => {
              setVistaAnteriorPermisos(vistaActual);
              setVistaActual('PERMISOS_CALIENTE');
            }}
            className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 ${
              vistaActual === 'PERMISOS_CALIENTE'
                ? 'bg-amber-500 text-slate-950 shadow-md font-black'
                : 'bg-slate-800/80 text-slate-300 hover:bg-slate-700 hover:text-white'
            }`}
            title="Ir a Permisos en Caliente"
          >
          ...
          <button
            type="button"
            onClick={() => setVistaActual('VISTA_CONSULTA_BITACORA')}
            className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 ${
              vistaActual === 'VISTA_CONSULTA_BITACORA'
                ? 'bg-amber-500 text-slate-950 shadow-md font-black'
                : 'bg-slate-800/80 text-slate-300 hover:bg-slate-700 hover:text-white'
            }`}
            title="Ir a Consulta de Bitácoras"
          >
            <Search className="w-3.5 h-3.5" />
            <span>Consulta Bitácoras</span>
          </button>

          <button
            type="button"
            onClick={() => setVistaActual('VISTA_CONSULTA_HOJA_TURNO')}
            className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 ${
              vistaActual === 'VISTA_CONSULTA_HOJA_TURNO'
                ? 'bg-amber-500 text-slate-950 shadow-md font-black'
                : 'bg-slate-800/80 text-slate-300 hover:bg-slate-700 hover:text-white'
            }`}
            title="Ir a Consulta Hoja de Turno"
          >
            <FileSpreadsheet className="w-3.5 h-3.5" />
            <span>Hoja Turno</span>
          </button>
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-slate-800 text-amber-400 hover:bg-slate-700 transition-colors cursor-pointer shrink-0"
            title={modoNocturno ? "Cambiar a Modo Diurno" : "Cambiar a Modo Nocturno"}
          >
            {modoNocturno ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4 text-indigo-400" />}
          </button>

          <button
            type="button"
            onClick={() => setMinimizadoDemoBar(true)}
            className="p-1.5 rounded-xl bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors cursor-pointer shrink-0 ml-0.5"
            title="Minimizar Navegador Demo"
          >
