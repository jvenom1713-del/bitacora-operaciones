...
export async function procesarArchivoCenCliente(file) {
  ...
  if (file.name.toLowerCase().endsWith('.zip')) {
    ...
    const excelFiles = Object.keys(zip.files).filter(n => n.toUpperCase().endsWith('.XLSX'));
    let prgName = excelFiles.find(n => n.toUpperCase().includes('PRG')) || excelFiles[0];
    let tcoName = excelFiles.find(n => n.toUpperCase().includes('PO'));
