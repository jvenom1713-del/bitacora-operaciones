...
export default function VistaConsultaHojaTurno({ 
  ...
  const descargarPdfResumenEjecutivo = async () => {
    ...
    const sisPromVal = (parametrosGeneracion?.sistemaProm && parametrosGeneracion.sistemaProm !== '--') ? `${parametrosGeneracion.sistemaProm} USD/MWh` : '55.8 USD/MWh';
    const potVal = (parametrosGeneracion?.potEspera && parametrosGeneracion.potEspera !== '--') ? `${parametrosGeneracion.potEspera} MW` : '4046 MW';
    const hrsVal = (parametrosGeneracion?.hrsCargaBase && parametrosGeneracion.hrsCargaBase !== '--') ? `${parametrosGeneracion.hrsCargaBase} hrs` : '1 hrs';
    const minTecVal = (parametrosGeneracion?.minTecnico && parametrosGeneracion.minTecnico !== '--') ? `${parametrosGeneracion.minTecnico} hrs` : '22 hrs';
    const cmgVal = (parametrosGeneracion?.costoMarginal && parametrosGeneracion.costoMarginal !== '--') ? `${parametrosGeneracion.costoMarginal} USD/MWh` : '50.6 USD/MWh';

    const fechaImpresionHoja = new Date().toLocaleDateString('es-CL', {
      day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit'
    });

    const jefeNombreActual = obtenerNombreJefeActual(usuarioActual, equipoTurnoState);
            ...
            <tr>
              <td style="vertical-align: middle;">
                ...
                <div style="font-size: 8.5px; color: #94a3b8; font-weight: 600;">
                  Fecha: ${fechaStr} | Turno: DIURNO | Cierre: ${fechaImpresionHoja} hrs
                </div>
              </td>
        ...
        <div style="padding: 10px 14px; font-size: 9.5px; line-height: 1.3;">
          ...
          <div style="margin-bottom: 10px;">
            ...
            <div style="background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 4px; padding: 8px 10px; font-size: 8.5px; color: #1e293b; line-height: 1.4; min-height: 40px; white-space: pre-line;">
              <strong style="color: #0369a1; display: inline-block; margin-right: 4px;">Día ${fechaStr}:</strong>${textos?.nuevaRencaDia1 || 'Solicitud de cierre de turno enviada por el operador.'}
            </div>
          </div>
              ...
              <tr>
                ...
                <td style="width: 30%; text-align: center; vertical-align: middle;">
                  ...
                  <div style="border: 3px double #15803d; border-radius: 50%; width: 115px; height: 115px; margin: 0 auto; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; color: #14532d; background: #86efac; font-family: 'Helvetica Neue', Arial, sans-serif; box-sizing: border-box; padding: 6px; box-shadow: inset 0 0 0 1.5px #4ade80;">
                    ...
                    <div style="font-size: 6px; font-weight: 800; margin-top: 2px; color: #14532d; font-family: monospace;">${fechaImpresionHoja}</div>
                  </div>
                </td>
